...
| <a name="input_transit_gateway_attachment_vpc_subnets_list"></a> [transit\_gateway\_attachment\_vpc\_subnets\_list](#input\_transit\_gateway\_attachment\_vpc\_subnets\_list) | List of vpc with subnets that will be attached to the transit gateway. Transit gateway will only be setup if this variable is not empty. | <pre>list(object({<br>    id         = string<br>    subnet_ids = list(string)<br>  }))</pre> | `[]` | no |
